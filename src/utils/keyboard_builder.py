...
class KeyboardBuilder:
    ...
    async def build_format_selection_keyboard(self, user_id: int, formats: list, chat_id: Optional[int] = None, is_admin: bool = False) -> InlineKeyboardMarkup:
        ...
        for fmt in formats:
            btn_text = await self.get_message(user_id, 'quality_format', chat_id, is_admin, quality=fmt['quality'], ext=fmt['ext'])
            keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"quality:{fmt['id']}{context}")])
            
        btn_best = await self.get_message(user_id, 'best_quality', chat_id, is_admin)
        keyboard.append([InlineKeyboardButton(btn_best, callback_data=f"quality:best{context}")])
        return InlineKeyboardMarkup(keyboard)
